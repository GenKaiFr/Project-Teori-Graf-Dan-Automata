...
<div class="max-w-2xl mx-auto">
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow">
        <div class="p-6 border-b border-gray-200 dark:border-gray-700">
            <h1 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Edit Rapat</h1>
        </div>
        ...
        <form action="{{ route('meetings.update', $meeting) }}" method="POST" class="p-6">
            ...
            <div class="mb-6">
                ...
                <textarea id="description" name="description" rows="3" 
                          class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">{{ old('description', $meeting->description) }}</textarea>
                @error('description')
                    <p class="text-red-500 dark:text-red-400 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select id="status" name="status" 
                            class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100">
                        @foreach(\App\Services\MeetingAutomataService::getAllStates() as $state)
                            <option value="{{ $state }}" {{ old('status', $meeting->status) == $state ? 'selected' : '' }}>
                                {{ $state }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    @foreach($participants as $participant)
                        <label class="flex items-center mb-2">
                            <input type="checkbox" name="participants[]" value="{{ $participant->id }}" 
                                   class="mr-2" {{ in_array($participant->id, old('participants', $meeting->participants->pluck('id')->toArray())) ? 'checked' : '' }}>
            ...
            <div class="flex justify-between">
                ...
                <button type="submit" id="submitBtn"
                    ...
                    <i class="fas fa-save mr-2"></i>Update Rapat
                </button>
            </div>
        </form>
    </div>
